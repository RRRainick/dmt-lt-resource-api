...
be copied to an integration-test host and run directly.
"""
...
def _delete_body(request_id: int, node_id: str, modality: str) -> dict[str, Any]:
    return {
        "timestamp_ms": int(time.time() * 1000),
        "request_id": request_id,
        "node_id": node_id,
...
def _positive_delete(
    ...
    result = client.request(
        "POST",
        "/modality/delete",
        json_body=_delete_body(request_id, node_id, modality),
    )
...
def run_robustness_tests(
    ...
    def post_robustness_cleanup() -> None:
        result = client.request(
            "POST",
            "/modality/delete",
            json_body=_delete_body(cleanup_request_id, args.node_id, args.modality),
        )
    ...
    try:
        ...
        def delete() -> dict[str, Any]:
            ...
            result = client.request(
                "POST",
                "/modality/delete",
                json_body=_delete_body(delete_request_id, args.node_id, args.modality),
            )
            if _response_code(result) == 0:
                deployed = False
